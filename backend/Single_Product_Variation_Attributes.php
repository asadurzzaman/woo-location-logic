...
function wpll_variation_settings_fields($loop, $variation_data, $variation)
{
    woocommerce_wp_select(
        array(
            'id' => '_wcll_country_restriction_type_role[' . $variation->ID . ']',
            'label' => __('Rule of Restriction', 'location-logic'),
            'default' => 'all',
            'class' => 'availability wpll_restricted_type',
            'style' => 'width:100%;',
            'value' => get_post_meta($variation->ID, '_wcll_country_restriction_type_role', true),
            'options' => array(
...
function save_wpll_variation_settings_fields($post_id)
{
    ...
    $wcll_country_restriction_type_role = $_POST['_wcll_country_restriction_type_role'][$post_id];
    if (!empty($wcll_country_restriction_type_role)) {
        update_post_meta($post_id, '_wcll_country_restriction_type_role', esc_attr($wcll_country_restriction_type_role));
    }
